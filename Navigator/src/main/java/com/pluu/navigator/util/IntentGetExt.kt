...
fun <T> bindExtra() = extraDelegate {
    it.intent.extras!!.get(DIRECTION_PARAMS_KEY) as T
}
...
fun <T, R> bindExtra(
    ...
    val args: T = it.intent.extras!!.get(DIRECTION_PARAMS_KEY) as T
    getter(args)
}
...
fun <T, R> optionalBindExtra(
    ...
    val args = it.intent.extras?.get(DIRECTION_PARAMS_KEY) as? T
    getter(args)
}
